...
print("=== TESTES: MOTOR ANALÍTICO ACADÊMICO ===\n")

print("1. PREDITOR DE DESEMPENHO")
historico = [5.5, 6.0, 6.2, 7.0]
projecao = PreditorDesempenho.regressao_linear_simples(historico)
print(f"Histórico: {historico} -> Projeção Linear Simples: {projecao}")

prob_reprov = PreditorDesempenho.regressao_logistica_probabilidade(nota_media=4.0, frequencia=60)
print(f"Prob. de Reprovação (Logística) p/ Nota 4 e 60% Freq: {prob_reprov}%")

tendencia = AnaliseTendencia.calcular_tendencia(7.0, 6.2)
print(f"Tendência de Melhora: {tendencia}%")
print(f"Previsão ARIMA Simplificada: {AnaliseTendencia.prever_arima_simplificado(historico)}\n")
...
dp_turma = KPITurma.desvio_padrao(notas_turma)
print(f"Desvio Padrão Turma: {dp_turma}")
print(f"Índice Homogeneidade (media 6.6, dev {dp_turma}): {KPITurma.indice_homogeneidade(6.62, dp_turma)}\n")

print("4. ESTATÍSTICA GERAL")
quartis = EstatisticaGeral.calcular_quartis(notas_turma)
print(f"Quartis da Turma: {quartis}")
y_freq = [40, 60, 70, 70, 75, 90, 95, 100] # frequencias relacionadas as notas
correlacao = EstatisticaGeral.matriz_de_correlacao_simples(y_freq, notas_turma)
print(f"Correlação de Pearson (Frequencia x Nota): {correlacao}")
print("\n=== FIM DOS TESTES ===")
